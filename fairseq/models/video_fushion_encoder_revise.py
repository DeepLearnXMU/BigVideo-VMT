...
@register_model("video_fushion_encoder_revise")
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = cls.build_fushion_encoder(args, src_dict, encoder_embed_tokens)
    ...
    @classmethod
    def build_fushion_encoder(cls, args, src_dict, embed_tokens):
        return TransformerFushionEncoder(args, src_dict, embed_tokens)
...
class TransformerFushionEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if args.fushion_encoder_layerdrop > 0.0:
            self.fushion_layers = LayerDropModuleList(p=self.encoder_layerdrop)
        else:
            self.fushion_layers = nn.ModuleList([])

        self.fushion_layers.extend(
            [self.build_fushion_encoder_layer(args) for i in range(args.fushion_encoder_layers)]
        )
        self.num_fushion_layers = len(self.fushion_layers)

        if args.fushion_encoder_normalize_before:
            self.fushion_layer_norm = LayerNorm(embed_dim)
        else:
            self.fushion_layer_norm = None
    ...
    def forward(
        ...
        bsz, video_length = video_padding_mask.size()[0], video_padding_mask.size()[1]
        bsz, text_length = text_padding_mask.size()[0], text_padding_mask.size()[1]

        x = merge.transpose(0, 1)  # reback to T x B + v len x C
        # encoder layers
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.feature_dim = args.encoder_embed_dim
        self.proj_dim = 128
    ...
    def extract_features_scriptable(
        ...
        if self.video_projection:
            ...
            text_hidden = (encoder_out.bottom_text_out * text_padding_mask.unsqueeze(-1)).sum(
                dim=1) / text_padding_mask.sum(
                dim=1).unsqueeze(
                -1)  # Bx H
            video_hidden = (encoder_out.bottom_video_out * video_padding_mask.unsqueeze(-1)).sum(
            ...
            return x, {"attn": [attn], "inner_states": inner_states, "bottom_text_h": encoder_out.bottom_text_out,
                       ...
                       "bottom_text_proj_h": bottom_text_proj_out,
                       "bottom_video_proj_h": bottom_video_proj_out,
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_base')
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_merge_before_pewln')
def video_fushion_encoder_revise_merge_before_pewln(args):
    # args for video MMT
    args.fushion_encoder_embed_dim = getattr(args, 'fushion_encoder_embed_dim', 512)
    args.fushion_encoder_ffn_embed_dim = getattr(args, 'fushion_encoder_ffn_embed_dim', 2048)
    args.fushion_encoder_layers = getattr(args, 'fushion_encoder_layers', 6)
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_merge_after_pewln')
def video_fushion_encoder_revise_merge_after_pewln(args):
    # args for video MMT
    args.fushion_encoder_embed_dim = getattr(args, 'fushion_encoder_embed_dim', 512)
    args.fushion_encoder_ffn_embed_dim = getattr(args, 'fushion_encoder_ffn_embed_dim', 2048)
    args.fushion_encoder_layers = getattr(args, 'fushion_encoder_layers', 6)
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_small_merge_before_pewln')
def video_fushion_encoder_revise_small_merge_before_pewln(args):
    # args for video MMT
    args.fushion_encoder_embed_dim = getattr(args, 'fushion_encoder_embed_dim', 512)
    args.fushion_encoder_ffn_embed_dim = getattr(args, 'fushion_encoder_ffn_embed_dim', 2048)
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_one_merge_before_pewln')
def video_fushion_encoder_revise_one_merge_before_pewln(args):
...
@register_model_architecture('video_fushion_encoder_revise',
                             'small_video_fushion_encoder_revise_one_merge_before_pewln')
def video_fushion_encoder_revise_one_merge_before_pewln(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 0)
...
@register_model_architecture('video_fushion_encoder_revise', 'video_fushion_encoder_revise_one_merge_after_pewln')
def video_fushion_encoder_revise_one_merge_after_pewln(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 0)
    # args for video MMT
    args.fushion_encoder_embed_dim = getattr(args, 'fushion_encoder_embed_dim', 512)
    args.fushion_encoder_ffn_embed_dim = getattr(args, 'fushion_encoder_ffn_embed_dim', 2048)
