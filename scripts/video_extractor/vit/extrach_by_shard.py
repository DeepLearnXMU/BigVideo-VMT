...
parser.add_argument('--tsv_index', default=None,type=int)
...
with th.no_grad():
    ...
    for k, data in enumerate(tqdm(loader)):
        ...
        # print("time",time.time()-s_time)
        # print(sdsa)
        # thread_pool.submit(save_fun, (names,batch_features))

        # if os.path.isfile(output_file):
        #     print(f'Video {output_file} already processed.')
        #     continue

        # safeguard output path before saving
        # np.savez(output_file, features=batch_features)

        # stime=time.time()
        # for i in range(B):
        #     output_file = os.path.join(output_dir,names[i]+".npz")
        #     # if os.path.isfile(output_file):
        #     #     print(f'Video {output_file} already processed.')
        #     #     continue
        #     min_ind = i
        ...
        #     batch_features = model.forward_features(video_batch,choice=args.choice)
        #     if args.choice=="patch":
        #         out = th.mean(batch_features,dim=0)

        #     batch_features = batch_features.cpu().numpy()
        #     if args.half_precision:
        #         batch_features = batch_features.astype('float16')
