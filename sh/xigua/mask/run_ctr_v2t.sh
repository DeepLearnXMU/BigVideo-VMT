...
name=${mask}ed20_arch${arch}_cri${cri}v2t_tgt${tgt_lang}_lr${lr}_wu${warmup}_mat${max_tokens}_me${max_epoches}_seed${seed}_gpu${gpu_num}_wd${weight_decay}_dp${dropout}_vtype${video_feat_type}_mvlen${max_vid_len}_ctrs${contrastive_strategy}_ctra${contrastive_align}_ctrw${contrastive_weight}_ctrt${contrastive_temperature}_patience${patience}
...
fairseq-train $local_data_dir \
  ...
  --weight-decay 0.1  \
  --clip-norm ${clip_norm}   \
  --criterion $criterion --label-smoothing 0.1 --report-modal-similarity --use_v2t_ctr  \
