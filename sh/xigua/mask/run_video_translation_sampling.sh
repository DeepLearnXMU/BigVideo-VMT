...
name=${mask}ed20_arch${arch}_cri${cri}_tgt${tgt_lang}_lr${lr}_wu${warmup}_mt${max_tokens}_me${max_epoches}_seed${seed}_gpu${gpu_num}_wd${weight_decay}_dp${dropout}_vtype${video_feat_type}_mvlen${max_vid_len}_vdp${video_dropout}_ss${sampling_strategy}_sf${sampling_frames}_patience${patience}
...
fairseq-train $local_data_dir \
  ...
  --sampling-strategy ${sampling_strategy} --sampling-frames ${sampling_frames} \
