...
with open(train,encoding='utf-8') as file:
    train=[" ".join(x.rstrip()) for x in file.readlines()]

with open(dev, encoding='utf-8') as file:
    dev = [" ".join(x.rstrip()) for x in file.readlines()]
print(train[:3])
print(dev[:3])
...
with open(output+"train.tok-char.zh", "w", encoding="utf-8") as w:
    for line in tqdm(train):
        w.write(line+ "\n")
with open(output+"dev.tok-char.zh", "w", encoding="utf-8") as w:
