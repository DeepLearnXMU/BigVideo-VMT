...
from fairseq import file_operation
...
class HdfsPathHandler(PathHandler):

    HDFS_PREFIX = "hdfs://"

    def __init__(self):
        self._cached_local_path = set()

    @staticmethod
    def _cache_path(p: str):
        home = file_operation.home()
        return file_operation.join_paths(home, "__fairseq_cached_dir", p[len(HdfsPathHandler.HDFS_PREFIX):])

    def _get_supported_prefixes(self) -> List[str]:
        return [self.HDFS_PREFIX]

    def _get_local_path(self, path: str, **kwargs: Any) -> str:
        local_path = self._cache_path(path)
        if path in self._cached_local_path and os.path.exists(local_path):
            logger.info(f"The path {path} has been copied to the local")
            return local_path
        with file_lock(local_path):
            if not os.path.exists(local_path):
                file_operation.sync_file(
                    target_path=local_path,
                    source_path=path,
                    overwrite_target=False,
                )
        self._cached_local_path.add(path)
        logger.info(f"Add the path to the cache {path}, since it has been copied to local.")
        return local_path
    ...
    def _open(self, path: str, mode: str = "r", buffering: int = -1, **kwargs: Any) -> Union[IO[str], IO[bytes]]:
        if mode in {"r", "rb"}:
            # For all read operation, all file will cached in local.
            local_path = self._get_local_path(path)
            return open(local_path, mode)
        else:
            # For write operation, the user should check only one process write.
            return file_operation.open_file(path, mode)
    ...
    def _exists(self, path: str, **kwargs: Any) -> bool:
        if file_operation.dir_exists(path) or file_operation.file_exists(path):
            return True
        else:
            return False

    def _isfile(self, path: str, **kwargs: Any) -> bool:
        return file_operation.file_exists(path)

    def _isdir(self, path: str, **kwargs: Any) -> bool:
        return file_operation.dir_exists(path)

    def _ls(self, path: str, **kwargs: Any) -> List[str]:
        return file_operation.ls_dir(path)

    def _mkdirs(self, path: str, **kwargs: Any) -> None:
        file_operation.make_dir(path)

    def _rm(self, path: str, **kwargs: Any) -> None:
        file_operation.remove_files(path)
...
PathManager.register_handler(HdfsPathHandler())
# PathManager.register_handler(NativePathHandler())
