...
with open("/root/terminology_evaluation/characters-master/stop_words","r",encoding="utf-8") as f:
...
def read_reference_data_wmt(lt, ls, lc):
	...
	assert len(linest) == len(liness) == len(linesc)
...
def compare_exact_window_overlap(hyp, ref, window):
	...
	for desired in desireds:
		...
		for j, listfs in enumerate(desireds[desired]):
			...
			for win in range(min(listfs) - 1, -1, -1):
				if reference_tokens[win] not in string.punctuation and reference_tokens[win] not in stop_words:
			...
			for k, ft in enumerate(fts):
				...
				for win in range(min(listft) - 1, -1, -1):
					if hyp_tokens[win] not in string.punctuation and hyp_tokens[win] not in stop_words:
	...
	return accuracy / matched if matched > 0 else 0,count
...
def exact_window_overlap_match(l2, references, outputs, ids, window, LOG):
	...
	for i, id in enumerate(ids):
		if id in references:
			if outputs[i] != "":
				acc1,count = compare_exact_window_overlap(outputs[i], references[id], window)
			acc += acc1
			whole += count
	
	print(f"\tExact Window Overlap Accuracy: {acc / whole}")
	with open(LOG, 'a') as op:
		op.write(f"\tExact Window Overlap Accuracy: {acc / whole}")
